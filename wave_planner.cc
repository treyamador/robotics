...
namespace {
	...
	const std::string ENV_PATH = "hospital_section.pnm";
	const std::string OUT_PATH = "waveoutput_";
	const std::string PNM_FILE_EXT = ".pnm";
	...
	PointXY& init, PointXY& dest, int gradient)
{
	...
	this->output_wavefront(
		ENV_PATH,OUT_PATH,PNM_FILE_EXT,gradient);
	this->prune_path();
	return path_;
}
...
bool Map::goal_reached(
	PointXY*& perimeter, int size, PointXY& goal) 
{
	for (int i = 0; i < size; ++i)
	...
	int gradient)
{
	...
	Point2D p = this->pixel_to_coordinate(path_.back());
	std::stringstream filename;
	filename << write_path << std::round(p.x_*10.0)/10.0 << "x" << 
		std::round(p.y_*10.0)/10.0 << std::setprecision(1) << ext;
...
Point2D Map::pixel_to_coordinate(PointXY& node) {
	return Point2D(
		(MAP_X_COORDINATES/this->width_pixel())*node.x_ -
			MAP_X_COORDINATES/2.0,
		MAP_Y_COORDINATES/2.0 - 
			(MAP_Y_COORDINATES/this->height_pixel())*node.y_);
}


PointXY Map::coordinate_to_pixel(Point2D& node) {
	return PointXY(
		static_cast<int>(std::round(this->width_pixel()/2.0 +
			(this->width_pixel()/MAP_X_COORDINATES)*node.x_)),
		static_cast<int>(std::round(this->height_pixel()/2.0 -
			(this->height_pixel()/MAP_Y_COORDINATES)*node.y_)));
}
...
Point2D Robot::pixel_to_coordinate(PointXY& node) {
	return map_.pixel_to_coordinate(node);
}


PointXY Robot::coordinate_to_pixel(Point2D& node) {
	return map_.coordinate_to_pixel(node);
}
